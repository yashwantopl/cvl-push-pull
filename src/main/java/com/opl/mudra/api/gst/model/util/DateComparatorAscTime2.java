...
public class DateComparatorAscTime2 implements Comparator<MomSales> {

    private final Logger logger = LoggerFactory.getLogger(DateComparatorAscTime2.class);

    @Override
    public int compare(MomSales o1, MomSales o2) {

        try {
            Date d1 = convertToDate(o1.getMonth());
            Date d2 = convertToDate(o2.getMonth());
            if (d1.after(d2))
                return 1;
