...
import com.opl.service.loans.domain.BankCWAuditTrailDomain;
...
public interface AdminDetailRepository extends JpaRepository<BankCWAuditTrailDomain, Long> {
    ...
    @Query(value = "SELECT prod.name FROM ``fp_product_master` prod WHERE prod.fp_product_id=:fpProductId AND prod.is_active = TRUE",nativeQuery = true)
    public String productList(@Param("fpProductId")Long fpProductId);

    @Query(value = "SELECT app.organisation_name FROM ``fs_corporate_applicant_details` app WHERE app.application_id=:applicationId",nativeQuery = true)
