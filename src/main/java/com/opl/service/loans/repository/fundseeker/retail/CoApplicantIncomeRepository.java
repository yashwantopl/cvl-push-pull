...
import com.opl.service.loans.domain.fundseeker.retail.CoApplicantIncomeDetail;
...
public interface CoApplicantIncomeRepository extends JpaRepository<CoApplicantIncomeDetail, Long>{
	...
	@Query(value="SELECT salary_income FROM  `loan_application`.`fs_retail_co_applicant_income_details` WHERE application_id = :applicationId AND proposal_mapping_id IS NULL AND is_active = TRUE ORDER BY YEAR DESC LIMIT 1", nativeQuery=true)
	public Double getCoApplicantLatestYearIncome(@Param("applicationId") Long applicationId);
