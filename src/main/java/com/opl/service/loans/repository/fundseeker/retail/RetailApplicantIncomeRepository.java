...
import com.opl.service.loans.domain.fundseeker.retail.RetailApplicantIncomeDetail;
...
public interface RetailApplicantIncomeRepository extends JpaRepository<RetailApplicantIncomeDetail, Long>{
	...
	@Query(value="SELECT salary_income FROM  `fs_retail_applicant_income_details` WHERE application_id =:applicationId AND proposal_mapping_id IS NULL AND is_active = TRUE ORDER BY YEAR DESC LIMIT 1", nativeQuery=true)
