...
public interface FpProductConditionsRepository extends JpaRepository<FpProductConditions, Long> {
    @Query("select new com.opl.mudra.api.loans.model.ProductConditionResponse(c.id,c.fpProductId,c.conditionName,c.isMandatory,c.isAllMandatory,c.allLogicalCondition,c.isActive,p.productId) from FpProductConditions c Left join ProductMasterTemp p on p.id = c.fpProductId where c.fpProductId =:productId")
