...
public interface IneligibleProposalDetailsRepository extends JpaRepository<IneligibleProposalDetails,Long> {
	...
	@Query(value = "SELECT ipd.application_id, cl.user_id, fs.name, usr.email, usr.mobile, ipd.created_date, ipd.branch_id, branch.name AS branchname, branch.contact_person_name, branch.telephone_no, branch.contact_person_number, org.organisation_name, lam.application_code, branch.code, branch.street_name, (SELECT state_name FROM `one_form`.`state` s WHERE s.id = branch.state_id), (SELECT city_name FROM `one_form`.`city` c WHERE c.id = branch.city_id), branch.premises_no, branch.contact_person_email\n" + 
			...
			"			LEFT JOIN `loan_application_mudra`.`fs_loan_application_master` lam ON lam.application_id = ipd.application_id\n" + 
    ...
    @Query(value = "SELECT inl.user_org_id FROM loan_application_mudra.`ineligible_proposal_details` inl WHERE inl.application_id =:applicationId", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM loan_application_mudra.`ineligible_proposal_details` inl WHERE SUBSTR(inl.gstin,3,10) =:gstin AND inl.`is_active` = TRUE", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM loan_application_mudra.ineligible_proposal_details WHERE application_id=:applicationId AND is_active=TRUE AND (is_sanctioned = TRUE OR is_disbursed = TRUE)", nativeQuery = true)
    public IneligibleProposalDetails getSanctionedByApplicationId(@Param("applicationId")Long applicationId);

	@Query(value = "SELECT * FROM loan_application_mudra.ineligible_proposal_details WHERE application_id=:applicationId AND is_active=TRUE AND (is_sanctioned = TRUE OR is_disbursed = TRUE) AND user_org_id <>:userOrgId", nativeQuery = true)
