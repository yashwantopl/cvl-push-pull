 ...
 */
@RestController
@RequestMapping("/purchase_property_details")
public class PurchasePropertyDetailsController {

	private static final Logger logger = LoggerFactory.getLogger(PurchasePropertyDetailsController.class);

	@Autowired
	private ReferenceRetailDetailsService referenceRetailDetailsService;

	@Autowired
	private PurchasePropertyDetailsService purchasePropertyDetailsService;
	...
	public ResponseEntity<LoansResponse> save(@RequestBody FrameRequest frameRequest, HttpServletRequest request,
		...
		try {
			...
			boolean response = purchasePropertyDetailsService.saveOrUpdate(frameRequest);
			if (response) {
				return new ResponseEntity<LoansResponse>(
	...
	public ResponseEntity<LoansResponse> getSalariedList(@PathVariable Long id, @PathVariable int applicationType, @PathVariable Long proposalId,
		...
		try {
			...
			List<PurchasePropertyDetailsRequest> response = purchasePropertyDetailsService
					.getPropertyDetailListByProposalId(proposalId, applicationType);
