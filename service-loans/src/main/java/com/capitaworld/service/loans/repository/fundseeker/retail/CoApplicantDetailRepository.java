...
public interface CoApplicantDetailRepository extends JpaRepository<CoApplicantDetail, Long> {
	...
	@Query("select cd.id from CoApplicantDetail cd where cd.applicationId.id =:applicationId and cd.isActive = true and (cd.isItrCompleted = true OR cd.isItrManual = true)")
	public List<Long> getCoAppIdsOfCoApplicantUploadedITR(@Param("applicationId") Long applicationId);
	...
	@Query("SELECT new com.capitaworld.service.loans.model.retail.CoApplicantRequest(cd.id,cd.firstName,cd.middleName,cd.lastName,cd.relationshipWithApplicant,cd.applicationId.id) FROM CoApplicantDetail cd WHERE cd.applicationId.id =:applicationId and cd.isActive = true ")
