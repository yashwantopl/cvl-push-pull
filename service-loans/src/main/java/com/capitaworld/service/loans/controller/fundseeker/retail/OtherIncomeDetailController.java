...
public class OtherIncomeDetailController {
	...
	public ResponseEntity<LoansResponse> save(@RequestBody FrameRequest frameRequest) {
		...
		try
		{
		boolean response = otherIncomeDetailService.saveOrUpdate(frameRequest);
		if (response) {
	...
	public ResponseEntity<LoansResponse> getList(@PathVariable Long id,@PathVariable int applicationType) {
		// request must not be null
		  try {
		   ...
		   List<OtherIncomeDetailRequest> response = otherIncomeDetailService.getOtherIncomeDetailList(id, applicationType);
