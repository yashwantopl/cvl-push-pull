...
public class OtherCurrentAssetDetailController {
	...
	public ResponseEntity<LoansResponse> save(@RequestBody FrameRequest frameRequest) {
		...
		try
		{
		boolean response = otherCurrentAssetDetailService.saveOrUpdate(frameRequest);
		if (response) {
	...
	public ResponseEntity<LoansResponse> getList(@PathVariable Long id,@PathVariable int applicationType) {
		// request must not be null
		  try {
		   ...
		   List<OtherCurrentAssetDetailRequest> response = otherCurrentAssetDetailService.getOtherCurrentAssetDetailList(id, applicationType);
