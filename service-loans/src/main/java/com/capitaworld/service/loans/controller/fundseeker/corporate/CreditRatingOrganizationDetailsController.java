...
private static final Logger logger = LoggerFactory.getLogger(CreditRatingOrganizationDetailsController.class);
	...
	public ResponseEntity<LoansResponse> save(@RequestBody FrameRequest frameRequest) {
		...
		try
		{
		boolean response = creditRatingOrganizationDetailsService.saveOrUpdate(frameRequest);
		if (response) {
	...
	public ResponseEntity<LoansResponse> getList(@PathVariable Long id) {
		// request must not be null
		  try {
		   ...
		   List<CreditRatingOrganizationDetailRequest> response = creditRatingOrganizationDetailsService.getcreditRatingOrganizationDetailsList(id);
