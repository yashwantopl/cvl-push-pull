...
public interface AssetsDetailsRepository extends JpaRepository<AssetsDetails, Long> {
	...
	@Query("select max(a.year) , a.receivableOtherThanDefferred, a.exportReceivables, a.inventory ,a.advanceToSupplierRawMaterials from AssetsDetails a where a.loanApplicationMaster.id =:applicationId  AND a.isActive=true AND a.financialYearlyStatement =:financialYearlyStatement ")
