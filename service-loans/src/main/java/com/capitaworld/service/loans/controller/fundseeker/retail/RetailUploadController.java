...
@RestController
@RequestMapping("/retail_upload")
public class RetailUploadController {
	...
	public ResponseEntity<LoansResponse> uploadProfileImage(@RequestPart("fileName") String fileName,
		...
		try {
			if (CommonUtils.isObjectNullOrEmpty(fileName) || CommonUtils.isObjectNullOrEmpty(mappingId)) {
				return new ResponseEntity<LoansResponse>(
						new LoansResponse("Invalid data or Requested data not found.", HttpStatus.BAD_REQUEST.value()),
						HttpStatus.OK);
			}

			DocumentResponse documentResponse = uploadProfilePic(applicationId, mappingId, fileName, multipartFiles);
	...
	public ResponseEntity<LoansResponse> getProfileImage(@PathVariable("applicationId") Long applicationId,@PathVariable("mappingId") Long mappingId,@PathVariable("userType")String userType,
			HttpServletRequest request) {
		try {
	...
	private DocumentResponse uploadProfilePic(Long applicantId, Long mappingId, String fileName,
