...
public class AssociatedConcernDetailController {
	...
	public ResponseEntity<LoansResponse> save(@RequestBody FrameRequest frameRequest) {
		...
		try
		{
		boolean response = associatedConcernDetaillService.saveOrUpdate(frameRequest);
		if (response) {
	...
	public ResponseEntity<LoansResponse> getList(@PathVariable Long id) {
		// request must not be null
		  try {
		   ...
		   List<AssociatedConcernDetailRequest> response = associatedConcernDetaillService.getAssociatedConcernsDetailList(id);
