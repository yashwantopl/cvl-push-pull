...
//				if(updatePayment) {
	...
	public boolean savePhese2DataToSidbi(Long applicationId, Long userId) {
		try {
			...
			logger.info("ratingResponse response ----->" + ratingResponse.getData());
			...
			IrrRequest req = MultipleJSONObjectHelper.getObjectFromMap((Map<String,Object>)ratingResponse.getData(), IrrRequest.class);
			logger.info("irr respo->"+req);
			BeanUtils.copyProperties(req,irrRequest);
