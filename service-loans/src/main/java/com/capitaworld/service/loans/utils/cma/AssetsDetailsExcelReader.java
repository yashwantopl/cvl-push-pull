...
public class AssetsDetailsExcelReader
{
    ...
    public static void run(Long storageDetailsId, XSSFSheet sheet, LoanApplicationMaster loanApplicationMaster, ApplicationProposalMapping applicationProposalMapping, AssetsDetailsRepository assetsDetailsRepository) {
        ...
        assetsMappingList.addAll(Arrays.asList(numbers));
        ...
        if (applicationProposalMapping.getBusinessTypeId() == CommonUtils.BusinessType.EXISTING_BUSINESS.getId()) {
            ...
            extractCellFromSheet(storageDetailsId, sheet, loanApplicationMaster,applicationProposalMapping, assetsMappingList, "E", String.valueOf(sheet.getRow(4).getCell(4).getNumericCellValue()), "Estimated", assetsDetailsRepository);
            j = 5;
        ...
        if (applicationProposalMapping.getProductId() != 15 && applicationProposalMapping.getProductId() != 1) {
            ...
            for (int i = 0; i < applicationProposalMapping.getTenure(); i++) {
                extractCellFromSheet(storageDetailsId, sheet, loanApplicationMaster,applicationProposalMapping, assetsMappingList, CellReference.convertNumToColString(sheet.getRow(4).getCell(j).getColumnIndex()), String.valueOf(sheet.getRow(4).getCell(j).getNumericCellValue()), "Projected", assetsDetailsRepository);
                j++;
            }
        }
    }
