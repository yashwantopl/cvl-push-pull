...
public interface OperatingStatementDetailsRepository  extends JpaRepository<OperatingStatementDetails, Long>{
   ...
   @Query("select a from OperatingStatementDetails a where a.loanApplicationMaster.id= :applicationId  AND a.year=(SELECT  max(a.year-1) FROM OperatingStatementDetails a WHERE a.loanApplicationMaster.id =:applicationId AND a.applicationProposalMapping.proposalId = NULL AND a.isActive=true AND a.financialYearlyStatement =:financialYearlyStatement)")
	 ...
	 */
	@Query(value="SELECT CAST(JSON_OBJECT('year', JSON_OBJECTAGG(YEAR, net_profit_or_loss)) AS CHAR) AS YEAR FROM `loan_application`.`fs_corporate_cma_operating_statement_details` WHERE application_id=:applicationId and is_active = true", nativeQuery=true)
	 ...
	 * @author rohit.chaudhary
	 */
	@Query(value="SELECT CAST(JSON_OBJECT('year', JSON_OBJECTAGG(YEAR, net_sales)) AS CHAR) AS YEAR FROM `loan_application`.`fs_corporate_cma_operating_statement_details` WHERE application_id=:applicationId and is_active = true", nativeQuery=true)
