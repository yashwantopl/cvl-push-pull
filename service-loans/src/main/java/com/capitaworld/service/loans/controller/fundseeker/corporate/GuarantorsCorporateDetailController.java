...
public class GuarantorsCorporateDetailController {
	...
	public ResponseEntity<LoansResponse> save(@RequestBody FrameRequest frameRequest) {
		...
		try
		{
		boolean response = guarantorsCorporateDetailService.saveOrUpdate(frameRequest);
		if (response) {
	...
	public ResponseEntity<LoansResponse> getList(@PathVariable Long id) {
		// request must not be null
		  try {
		   ...
		   List<GuarantorsCorporateDetailRequest> response = guarantorsCorporateDetailService.getGuarantorsCorporateDetailList(id);
