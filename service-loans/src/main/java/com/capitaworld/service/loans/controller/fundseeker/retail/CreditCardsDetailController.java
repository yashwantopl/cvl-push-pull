...
public class CreditCardsDetailController {
	...
	public ResponseEntity<LoansResponse> save(@RequestBody FrameRequest frameRequest) {
		...
		try
		{
		boolean response = creditCardsDetailService.saveOrUpdate(frameRequest);
		if (response) {
	...
	public ResponseEntity<LoansResponse> getList(@PathVariable Long id,@PathVariable int applicationType) {
		// request must not be null
		  try {
		   ...
		   List<CreditCardsDetailRequest> response = creditCardsDetailService.getExistingLoanDetailList(id, applicationType);
