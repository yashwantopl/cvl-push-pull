...
public class BankAccountHeldDetailController {
	...
	public ResponseEntity<LoansResponse> save(@RequestBody FrameRequest frameRequest) {
		...
		try
		{
		boolean response = bankAccountHeldDetailService.saveOrUpdate(frameRequest);
		if (response) {
	...
	public ResponseEntity<LoansResponse> getList(@PathVariable Long id,@PathVariable int applicationType) {
		// request must not be null
		  try {
		   ...
		   List<BankAccountHeldDetailsRequest> response = bankAccountHeldDetailService.getExistingLoanDetailList(id, applicationType);
