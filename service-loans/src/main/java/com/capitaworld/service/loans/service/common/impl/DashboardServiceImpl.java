	...
	public DashboardProfileResponse getBasicProfileInfo(Long applicationId, Long userId) throws Exception {
		...
		if (userMainType == CommonUtils.UserMainType.CORPORATE) {
			...
			if (!CommonUtils.isObjectNullOrEmpty(corporateApplicantDetail.getRegisteredCityId())) {
				dashboardProfileResponse.setCity(CommonDocumentUtils.getCity(corporateApplicantDetail.getRegisteredCityId(),environment));
			}

			// Setting State Value
			if (!CommonUtils.isObjectNullOrEmpty(corporateApplicantDetail.getRegisteredStateId())) {
				dashboardProfileResponse.setState(CommonDocumentUtils.getState(corporateApplicantDetail.getRegisteredStateId().longValue(),environment));
			}

			// Country State Value
			if (!CommonUtils.isObjectNullOrEmpty(corporateApplicantDetail.getRegisteredCountryId())) {
				dashboardProfileResponse.setCountry(CommonDocumentUtils.getCountry(corporateApplicantDetail.getRegisteredCountryId().longValue(),environment));
			}
			...
			if (!CommonUtils.isObjectNullOrEmpty(retailApplicantDetail.getAddressCity())) {
				dashboardProfileResponse.setCity(CommonDocumentUtils.getCity(retailApplicantDetail.getAddressCity(),environment));
			}

			// Setting State Value
			if (!CommonUtils.isObjectNullOrEmpty(retailApplicantDetail.getAddressState())) {
				dashboardProfileResponse.setState(CommonDocumentUtils.getState(retailApplicantDetail.getAddressState().longValue(),environment));
			}

			// Country State Value
			if (!CommonUtils.isObjectNullOrEmpty(retailApplicantDetail.getAddressCountry())) {
				dashboardProfileResponse.setCountry(CommonDocumentUtils.getCountry(retailApplicantDetail.getAddressCountry().longValue(),environment));
			}
			
			String name = !CommonUtils.isObjectNullOrEmpty(retailApplicantDetail.getFirstName()) ? retailApplicantDetail.getFirstName() +" " : "NA " ;
			name = name.concat(!CommonUtils.isObjectNullOrEmpty(retailApplicantDetail.getLastName()) ? retailApplicantDetail.getLastName() : "NA");
