...
public interface CspCodeRepository extends JpaRepository<CspCode,Long> {
	...
	@Query(value="SELECT count(cs.id) from loan_application.csp_code cs where LOWER(loan_application.fn_remove_special_chars(cs.description)) =:code and cs.org_id =:orgId")
