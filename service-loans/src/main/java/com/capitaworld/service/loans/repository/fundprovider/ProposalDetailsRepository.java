...
public interface ProposalDetailsRepository extends JpaRepository<ProposalDetails,Long>{
    ...
    @Query(value = "SELECT lm.applicationId,cap.organisationName,lm.applicationCode,lm.businessTypeId from ProposalDetails pd,CorporateApplicantDetail cap,ApplicationProposalMapping lm where pd.fpProductId =:fpProductId and pd.proposalStatusId.id =:proposalStatusId and pd.branchId IN :branchIdList and pd.isActive = true and cap.applicationId.id = pd.applicationId and cap.applicationId.id = pd.applicationId and cap.applicationId.id = lm.applicationId")
