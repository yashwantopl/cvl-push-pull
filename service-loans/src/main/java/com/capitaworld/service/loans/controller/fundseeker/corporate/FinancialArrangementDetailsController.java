...
public class FinancialArrangementDetailsController {
	...
	public ResponseEntity<LoansResponse> save(@RequestBody FrameRequest frameRequest) {
		...
		try
		{
		boolean response = financialArrangementDetailsService.saveOrUpdate(frameRequest);
		if (response) {
	...
	public ResponseEntity<LoansResponse> getList(@PathVariable Long id) {
		// request must not be null
		  try {
		   ...
		   List<FinancialArrangementsDetailRequest> response = financialArrangementDetailsService.getFinancialArrangementDetailsList(id);
