...
private static final Logger logger = LoggerFactory.getLogger(FutureFinancialEstimatesDetailsController.class);
	...
	public ResponseEntity<LoansResponse> save(@RequestBody FrameRequest frameRequest) {
		...
		try
		{
		boolean response = futureFinancialEstiamateDetailsService.saveOrUpdate(frameRequest);
		if (response) {
	...
	public ResponseEntity<LoansResponse> getList(@PathVariable Long id) {
		// request must not be null
		  try {
		   ...
		   List<FutureFinancialEstimatesDetailRequest> response = futureFinancialEstiamateDetailsService.getFutureFinancialEstimateDetailsList(id);
