...
public interface FinancialArrangementDetailsRepository extends JpaRepository<FinancialArrangementsDetail, Long> {
	...
	@Query("select sum(o.outstandingAmount) from FinancialArrangementsDetail o where o.applicationId.id =:id and o.isActive = true and o.directorBackgroundDetail IS NULL and LOWER(o.loanType) IN (:loanType) and o.outstandingAmount IS NOT NULL and o.outstandingAmount > 0")
	...
	@Query("select sum(o.outstandingAmount) from FinancialArrangementsDetail o where o.applicationId.id =:id and o.isActive = true and o.directorBackgroundDetail IS NULL and o.outstandingAmount IS NOT NULL and o.outstandingAmount > 0 and o.isManuallyAdded = true")
