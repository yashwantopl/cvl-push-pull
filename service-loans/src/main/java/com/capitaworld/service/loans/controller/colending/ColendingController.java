...
public class ColendingController {
    ...
    @PostMapping(value = "/get/allRatio", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<LoansResponse> getCoOriginationAllRatio(@RequestBody NhbsApplicationRequest nhbsApplicationRequest, HttpServletRequest request){
        try {
            Long userId = (Long) request.getAttribute(CommonUtils.USER_ID);
            if(CommonUtils.isObjectNullOrEmpty(nhbsApplicationRequest)){
                logger.warn(CommonUtils.INVALID_DATA_OR_REQUESTED_DATA_NOT_FOUND);
            ...
            nhbsApplicationRequest.setUserId(userId);
            List<CoLendingRequest> coLendingRequestList = coLendingService.getCoOriginationAllRatio(nhbsApplicationRequest);
            LoansResponse loansResponse = new LoansResponse();
            if(!CommonUtils.isListNullOrEmpty(coLendingRequestList)){
                loansResponse.setMessage(CommonUtils.DATA_FOUND);
            }else{
            ...
            logger.error("Error in getCoOriginationAllRatio() : ", e);
            return new ResponseEntity<LoansResponse>(
                    new LoansResponse(CommonUtils.SOMETHING_WENT_WRONG, HttpStatus.INTERNAL_SERVER_ERROR.value()),
                    HttpStatus.INTERNAL_SERVER_ERROR);
        }
