...
public class LoanRepositoryImpl implements LoanRepository {
	...
	public List<Double> getIncomeOfItrOf3Years(Long applicationId) {
		return (List<Double>)entityManager.createNativeQuery("SELECT appInc.`year`,appInc.`salary_income` FROM `loan_application`.`fs_retail_applicant_income_details` appInc WHERE appInc.`application_id` =:applicationId AND appInc.`proposal_mapping_id` IS NULL AND `appInc`.`salary_income` IS NOT NULL ORDER BY appInc.`year` DESC ")
