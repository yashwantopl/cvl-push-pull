...
-- DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
	...
	WHILE @k<@applicationIdCount DO 				
		...
		SET @i=0;
			...
			IF ((@stageId = 7 OR @stageId = 9 OR @stageId = 211 OR @stageId = 210 )  AND @status = 3 ) THEN
			...
			ELSEIF ((@stageId = 4 AND @status = 6) OR (@stageId = 207 AND @status = 6) ) THEN
				...
				IF (@businessTypeId = 3 OR @businessTypeId = 5) THEN
					SET @companyNameQuery:= CONCAT('select CONCAT(first_name," ",middle_name," ",last_name) into @companyName from loan_application.fs_retail_applicant_details where application_id=',@applicationId,' limit 1');
					PREPARE stmtCompanyName FROM @companyNameQuery;
					EXECUTE stmtCompanyName;
			...
			ELSEIF(((@stageId != 7 AND  @stageId != 9) OR (@stageId != 211 AND @stageId != 210)) AND (@status IN (1,2,3,4))) THEN
