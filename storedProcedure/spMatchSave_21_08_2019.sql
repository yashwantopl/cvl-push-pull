...
-- ===================================== Start Get Product Parameter Variable ===================================== --
   SET @productParaQuery:= CONCAT('SELECT
	 ...
	 ,is_collateral_coverage_display,is_collateral_coverage_mandatory
	 ,is_conservative_ratio_display,is_conservative_ratio_mandatory
	 ,is_main_individual_cibil_display, is_main_individual_cibil_mandatory, main_individual_cibil
	 ,is_Commercial_cibil_for_3_month_display, is_Commercial_cibil_for_3_month_mandatory, commercial_cibil_for_3_month
	  INTO
	 ...
	 ,@isDSCRDisplay,@isDSCRMandatory
	 ,@isMainIndividualCibilDisplay, @isMainIndividualCibilMandatory, @mainIndividualCibil
	 ,@isDpdDisplay, @isDpdMandatory, @dpdFP
	 ...
	 SET @minAdditionalLoanFP= NULL ,@additionalLoanAmountFS= NULL ,@maxAdditionalLoanFP= NULL ,@minTotalLoanFP= NULL,@existingLoanAmount= NULL,@maxTotalLoanFP= NULL,@wcRenewalStatus=NULL,@fpArrangmentId=NULL,@loanArrangementId=NULL; --  Investment exiting and addition loan amount changes
	 ...
	 SET @isDSCRMatched= FALSE ,@minDSCRValFP= NULL ,@maxDSCRValFP= NULL,@fpDSCR=NULL,@fsDSCR=NULL; --  DSCR
	 SET @isBureauScoreMatch = FALSE;  -- Minimum Bureau Score - Main Director/ Partner/ Proprietor
	 SET @isDpdMatch = FALSE, @dpdFS = NULL; -- Commercial Bureau - Maximum DPDs (last 3 months)
...
-- 	EXECUTE stmt ;
SET @currentRatioFSQuery:= CONCAT('SELECT(SELECT IF(ISNULL(total_current_assets),0,total_current_assets) FROM fs_corporate_cma_assets_details WHERE application_id = ',@applicationId,' AND proposal_mapping_id IS NULL AND is_active = 1 AND YEAR = ',@itrYear,' ORDER BY id DESC LIMIT 1 )/
					 (SELECT IF(ISNULL(total_current_liabilities),0,total_current_liabilities) FROM fs_corporate_cma_liabilities_details WHERE application_id = ',@applicationId,' AND proposal_mapping_id IS NULL AND is_active = 1 AND YEAR = ',@itrYear,' ORDER BY id DESC LIMIT 1) into @currentRatioFS');
	PREPARE stmt FROM @currentRatioFSQuery ;
	EXECUTE stmt ;
IF (@isCurrentratioDisplay = TRUE) THEN

	IF (@currentRatioFS='-0')THEN
		SET @currentRatioFS=0;
	END IF;
	SET @currentRatioAssetValFs := CONCAT('SELECT (IF(ISNULL(total_current_assets),0,total_current_assets)) INTO @currentRatioAssetFS FROM fs_corporate_cma_assets_details WHERE application_id = ',@applicationId,' AND proposal_mapping_id IS NULL AND is_active = 1
						AND YEAR= ',@itrYear,' ORDER BY id DESC LIMIT 1');
	PREPARE stmt FROM @currentRatioAssetValFs ;
	EXECUTE stmt ;
	SET @currentRatioLiabilitiesValFs := CONCAT('SELECT (IF(ISNULL(total_current_liabilities),0,total_current_liabilities)) INTO @currentRatioLiabilitiesFS FROM fs_corporate_cma_liabilities_details WHERE application_id = ',@applicationId,' AND proposal_mapping_id IS NULL AND is_active = 1
						AND YEAR= ',@itrYear,' ORDER BY id DESC LIMIT 1');
	PREPARE stmt FROM @currentRatioLiabilitiesValFs ;
	EXECUTE stmt ;
	IF (@currentRatioAssetFS < 0 AND @currentRatioLiabilitiesFS < 0) THEN -- //- infinity
		SET @isCurrentRatioMatch = FALSE;
	ELSEIF(@currentRatioAssetFS < 0 AND @currentRatioLiabilitiesFS = 0) THEN -- //- infinity
		SET @isCurrentRatioMatch = FALSE;
	ELSEIF(@currentRatioAssetFS < 0 AND @currentRatioLiabilitiesFS > 0) THEN -- //- infinity
		SET @isCurrentRatioMatch = FALSE;
	ELSEIF(@currentRatioAssetFS = 0 AND @currentRatioLiabilitiesFS > 0) THEN -- //- infinity
		SET @isCurrentRatioMatch = FALSE;
	ELSEIF(@currentRatioAssetFS = 0 AND @currentRatioLiabilitiesFS = 0) THEN -- //+ infinity
		SET @isCurrentRatioMatch = TRUE;
	ELSEIF(@currentRatioAssetFS > 0 AND @currentRatioLiabilitiesFS = 0) THEN -- //+ infinity
		SET @isCurrentRatioMatch = TRUE;
	ELSE
		SET @currentRatioMatchQuery:= CONCAT('SELECT CASE WHEN max_current_ratio=5
						THEN
						(SELECT (',@currentRatioFS,') >= (SELECT min_current_ratio FROM
						',@fpTableName,' WHERE fp_product_id = ',@fpProdId,'))
						ELSE
						(SELECT (',@currentRatioFS,') BETWEEN (SELECT min_current_ratio FROM ',@fpTableName,'
...
SET @dpdQuery:= CONCAT('SELECT UNCOMPRESS(lg.response) INTO @dpdArray FROM cibil.`cibil_audit_log_details` lg,`cibil`.`cibil_mstr` ms
	...
	SET @value:= REPLACE(REPLACE(REPLACE(@dpdArray, '[', ''), ']', ''),'"','');
	...
	ELSE
		WHILE (LOCATE(',', @value) > 0) DO
		      SET @V_DESIGNATION = SUBSTRING(@value,1, LOCATE(',',@value)-1);
		      SET @str_date = SUBSTRING(@V_DESIGNATION, 1, LOCATE('|',@V_DESIGNATION)-1);
		      SET @date = STR_TO_DATE(CONCAT(DAYOFMONTH(NOW())," ",@str_date), "%d %b %Y");
		      IF (TIMESTAMPDIFF(MONTH, @date, NOW()) <= 3) THEN
			SET @dpd = SUBSTRING(@V_DESIGNATION, LOCATE('|',@V_DESIGNATION)+1);
			INSERT INTO dpd_temp_tbl(dpd) VALUES(@dpd);
		      END IF;

		      SET @value = SUBSTRING(@value, LOCATE(',',@value) + 1);
	--	      SELECT @date, @dpd, @str_date, TIMESTAMPDIFF(MONTH, @date, NOW()) FROM DUAL;
		END WHILE;

		IF (LOCATE(',', @value) = 0) THEN
			 SET @valueOne = SUBSTRING(@value, LOCATE(']',@value) + 1);
			 IF (@valueOne IS NOT NULL AND @valueOne != '' ) THEN
				SET @str_date = SUBSTRING(@value, 1, LOCATE('|',@value)-1);
				SET @date = STR_TO_DATE(CONCAT(DAYOFMONTH(NOW())," ",@str_date), "%d %b %Y");

				IF (TIMESTAMPDIFF(MONTH, @date, NOW()) <= 3) THEN
					SET @dpd = SUBSTRING(@valueOne, LOCATE('|',@valueOne)+1);
					INSERT INTO dpd_temp_tbl(dpd) VALUES(@dpd);
				END IF;
...
ELSE
	...
	IF (@loanArrangementId = 3) THEN
		...
		SET @existingLoanAmountQuery := CONCAT('select IF(ISNULL(max_loan_amount),0,max_loan_amount) INTO @existingLoanAmount from application_product_audit where application_id=',@applicationId,' and fp_product_id=',@fpProdId);
		...
		SET @existingLoanAmount = @maxTotalLoanFP;
...
IF (@isCreditSummationDisplay = TRUE) THEN
	...
	IF (@gstin IS NOT NULL) THEN
		...
		IF (@hitSales IS NULL) THEN
		 	SET @projectedSalesQuery:=CONCAT('SELECT projected_sales INTO @projectedSales FROM gst.gst_mapping_table WHERE gstin =',@gstin,' ORDER BY id DESC LIMIT 1');
		 	PREPARE stmt FROM @projectedSalesQuery;
		 	EXECUTE stmt;
...
IF (@isCollateralCoverageNewDisplay = TRUE) THEN
	...
	IF (@haveCollateralSecurity IS NOT NULL AND @collateralAmt IS NOT NULL AND @haveCollateralSecurity=TRUE) THEN
		...
		IF (@collateralCoverageNewValFS IS NULL) THEN
			SET @collateralCoverageNewValFS = 0.00;
		ELSE
			SET @collateralCoverageNewValFS = ROUND(@collateralCoverageNewValFS,2);
		END IF;
		-- SELECT @collateralAmt,@loanAmout, @collateralCoverageNewValFS FROM DUAL;
...
-- =========================================== START DSCR Variable ==========================================  --
IF (@isDSCRDisplay = TRUE) THEN

	SET @fsDSCRQuery = CONCAT('SELECT fsDSCR INTO @fsDSCR FROM loan_application.application_product_audit WHERE fp_product_id =',@fpProdId,' AND application_id =',@applicationId,' AND is_active = TRUE');
	PREPARE stmt FROM @fsDSCRQuery;
	EXECUTE stmt;

	SET @fpDSCRQuery = CONCAT('SELECT min_conservative_ratio,max_conservative_ratio INTO @minDSCRValFP,@maxDSCRValFP FROM ',@fpTableName ,' WHERE fp_product_id =',@fpProdId);
	PREPARE stmt FROM @fpDSCRQuery;
	EXECUTE stmt;

	SET @isDSCRMatched=FALSE;
	SET @fpDSCR = '-';
	IF ((@minDSCRValFP IS NOT NULL) AND (@maxDSCRValFP IS NOT NULL)) THEN
		SET @fpDSCR = CONCAT(@minDSCRValFP,'-',@maxDSCRValFP);

		IF (@minDSCRValFP =0 AND @maxDSCRValFP=5)THEN
			SET @isDSCRMatched=FALSE;
		ELSEIF (@minDSCRValFP=0 AND @fsDSCR <= @maxDSCRValFP) THEN
			SET @isDSCRMatched=TRUE;
		ELSEIF (@maxDSCRValFP=5 AND @fsDSCR >= @minDSCRValFP)THEN
			SET @isDSCRMatched=TRUE;
		ELSE
			IF ((@minDSCRValFP IS NOT NULL) AND (@maxDSCRValFP IS NOT NULL) AND (@fsDSCR <= @maxDSCRValFP) AND (@fsDSCR >= @minDSCRValFP)) THEN
				SET @isDSCRMatched=TRUE;
			END IF;
		END IF;
	END IF;
ELSE
	SET @isDSCRDisplay=FALSE;
	SET @isDSCRMandatory=FALSE;
	SET @isDSCRMatched=FALSE;
	SET @DSCRValFP='-';
END IF;
-- =========================================== END DSCR Variable ============================================ --
-- ========================================= addtion on 17-07-2019 ========================================== --

-- ======================= Start Minimum Bureau Score - Main Director/ Partner/ Proprietor Match ==================== --
IF (@isMainIndividualCibilDisplay= TRUE) THEN

	SET @bureauScoreFsQuery = CONCAT('SELECT MIN(csld.actual_score) into @bureauScoreFS FROM cibil.cibil_score_log_details AS csld JOIN cibil.cibil_mstr AS cm ON cm.cibil_id = csld.cibil_id JOIN loan_application.fs_corporate_director_background_details AS dbd ON dbd.pan_no = csld.pan WHERE cm.application_id = ',@applicationId,' AND dbd.is_main_director = TRUE AND cm.is_active =TRUE');
	PREPARE stmt FROM @bureauScoreFsQuery ;
	EXECUTE stmt ;

	IF (@bureauScoreFS >= @mainIndividualCibil) THEN
		SET @isBureauScoreMatch = TRUE;
	ELSE
		SET @isBureauScoreMatch = FALSE;
	END IF;

END IF;
-- ======================= End Minimum Bureau Score - Main Director/ Partner/ Proprietor Match ==================== --

-- ======================= Start Commercial Bureau - Maximum DPDs (last 3 months) =========================== --
IF (@isDpdDisplay= TRUE) THEN
	SET @dpdQuery:= CONCAT('SELECT UNCOMPRESS(lg.response) into @dpdArray FROM cibil.cibil_audit_log_details AS lg JOIN cibil.cibil_mstr AS ms ON lg.cibil_id = ms.cibil_id WHERE lg.response IS NOT NULL AND (lg.request_type = "MSME_INDIVIDUAL_DPD" OR lg.request_type = "MSME_COMPANY_DPD") AND OCTET_LENGTH(lg.response) > 15 AND ms.application_id = ',@applicationId,' AND STATUS = "SUCCESS" ORDER BY lg.cibil_id DESC LIMIT 1');
	PREPARE stmt FROM @dpdQuery ;
	EXECUTE stmt ;
	...
	SET @commercialCibilFSQuery:= CONCAT('SELECT IF(MAX(dpd) IS NULL, 0, MAX(dpd)) into @dpdFS FROM dpd_temp_tbl');
	PREPARE stmt FROM @commercialCibilFSQuery ;
	EXECUTE stmt ;
	...
	IF (@dpdFS <= @dpdFP) THEN
 		SET @isDpdMatch = TRUE;
 	ELSE
 		SET @isDpdMatch = FALSE;
 	END IF;

END IF;
-- ======================= End Commercial Bureau - Maximum DPDs (last 3 months) =========================== --
-- ========================================= addtion on 17-07-2019 ========================================== --
...
SELECT
	 ...
	 ,@isDSCRMatched,@fsDSCR,@minDSCRValFP,@maxDSCRValFP,@fpDSCR
	 ,@isIndustrySectorDisplay,@isIndustrySectorMandatory
	 ...
	 ,@isMainIndividualCibilDisplay, @isMainIndividualCibilMandatory, @isBureauScoreMatch, @mainIndividualCibil, @bureauScoreFS
	 ,@isDpdDisplay, @isDpdMandatory, @isDpdMatch, @dpdFP, @dpdFS
	FROM DUAL;
